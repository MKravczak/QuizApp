...
QuizApp wykorzystuje PostgreSQL 14 z podziałem na schematy odpowiadające mikroserwisach:

```sql
-- Inicjalizacja schematów
CREATE SCHEMA IF NOT EXISTS users;       -- User Service
CREATE SCHEMA IF NOT EXISTS flashcards;  -- Flashcard Service  
CREATE SCHEMA IF NOT EXISTS quizzes;     -- Quiz Service
CREATE SCHEMA IF NOT EXISTS statistics;  -- Statistics Service
```
