...
public class AntiPostmanFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        log.info("AntiPostmanFilter - DISABLED - allowing all requests through");
        ...
        log.info("AntiPostmanFilter - enabled flag: {}", antiPostmanEnabled);

        // Sprawdź czy filtr jest włączony
        if (!antiPostmanEnabled) {
            log.info("AntiPostmanFilter is disabled - skipping all checks");
            chain.doFilter(request, response);
            return;
        }

        // Zezwól na endpointy health check
